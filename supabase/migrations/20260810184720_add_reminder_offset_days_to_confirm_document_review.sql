CREATE OR REPLACE FUNCTION public.confirm_document_review(
  ...
  p_reminder_offset_days integer DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
  ...
  IF p_due_date IS NOT NULL THEN
    INSERT INTO public.deadlines (
      user_id, document_id, title, due_date, amount, recurrence, reminder_offset_days
    )
    VALUES (
      auth.uid(), p_document_id, p_title, p_due_date, p_amount, 'none', p_reminder_offset_days
    );
  END IF;
END;
$$;